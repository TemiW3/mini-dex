...
import { MINIDEX_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof MINIDEX_PROGRAM_ADDRESS,
  TAccountMinidex extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountMinidex extends string
        ? WritableAccount<TAccountMinidex>
        : TAccountMinidex,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountMinidex extends string = string> = {
  minidex: Address<TAccountMinidex>;
};

export function getDecrementInstruction<
  TAccountMinidex extends string,
  TProgramAddress extends Address = typeof MINIDEX_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountMinidex>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountMinidex> {
  // Program address.
  const programAddress = config?.programAddress ?? MINIDEX_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    minidex: { value: input.minidex ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.minidex)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountMinidex>;
